 ...
 */
void I2C_Init() { TWBR = BITRATE(TWSR = 0x00); }
...
/*
 * @brief: Writes byte to an slave
 * @inputs:
 *   @address: This is the slave address to write
 *   @subAddress: The register to be written
 *   @data: Byte to write on the address
 */
i2c_state writeByte(uint8_t address, uint8_t subAddress, uint8_t data) {
  I2C_Start_Wait(address);
  if (failed == I2C_Write(subAddress)) {
    return failed;
  }
  if (failed == I2C_Write(data)) {
    return failed;
  }
  I2C_Stop();
  return ready;
